...
internal static class ILTemplate2
{
...
#endif
    {
        ...
        var assembly = Common2.ReadExistingAssembly(assemblyName);
        if (assembly is not null)
        {
            return assembly;
        }

        Common2.Log("Loading assembly '{0}' into the current context", assemblyName);

        assembly = Common2.ReadFromEmbeddedResources(assemblyNames, symbolNames, assemblyName);
        if (assembly is null)
        {
            lock (nullCacheLock)
            {
